...
/* =========================================================
   ...
   The signature element — the profile photo reads as an
   ID badge suspended on a thin cable directly beneath the
   navbar, gently swaying, rather than a normal avatar
   sitting inside the hero copy.
========================================================= */

function HangingProfile({ enableMotion }: { enableMotion: boolean }) {
  return (
    <div className="relative flex flex-col items-start">
      {/* Cable dropping from the navbar — flush against the top edge,
          no gap, so it reads as attached rather than floating */}
      <motion.div
        initial={enableMotion ? { scaleY: 0, opacity: 0 } : false}
        animate={enableMotion ? { scaleY: 1, opacity: 1 } : undefined}
        transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
        style={{ transformOrigin: "top" }}
        className="relative z-0 -mb-15 -mt-0 h-20 w-px bg-gradient-to-b from-[#D8CFBC] via-[#565449] to-[#302F29]"
      >
        {/* Anchor ring — half-bleeds above the section edge so it
            visually clips onto the navbar's bottom border */}
        <span className="absolute -top-3 left-1 h-2 w-2 -translate-x-2 rounded-full border border-[#D8CFBC] bg-[#11120D]" />
      </motion.div>

      {/* Badge */}
      <motion.div
        initial={enableMotion ? { opacity: 0, y: -20, rotate: -5 } : false}
        animate={enableMotion ? { opacity: 1, y: 0, rotate: -2 } : undefined}
        transition={{ duration: 0.7, delay: 0.35, ease: [0.22, 1, 0.36, 1] }}
        whileHover={enableMotion ? { rotate: 0, scale: 1.0 } : undefined}
        className="group relative"
      >
        <motion.div
          animate={enableMotion ? { rotate: [-2, 1.5, -2] } : undefined}
...
export default function Hero() {
  ...
  return (
    <section
      id="home"
      className="relative min-h-screen overflow-x-hidden bg-[#11120D] pt-16 text-[#FFFBF4]"
    >
      {/* Ambient glow accents */}
      <div className="pointer-events-none absolute -left-32 top-40 h-[380px] w-[380px] rounded-full bg-[#D8CFBC]/10 blur-[130px]" />

      <div className="relative mx-auto grid max-w-7xl items-start gap-12 px-6 pb-16 lg:grid-cols-2 lg:items-center lg:px-8">
        {/* =================================================
            LEFT — hanging badge + intro
        ================================================= */}
        <div className="text-left">
          ...
          <div className="mt-10">
            ...
            <h1 className="max-w-xl text-5xl font-semibold leading-tight tracking-tight sm:text-6xl lg:text-5xl">
              Sarvesh{" "}
            ...
            <h3 className="mt-3 text-3l font-medium text-[#D8CFBC]">
        ...
        {/* =================================================
            RIGHT — 3D SB + orbiting technologies (unchanged)
        ================================================= */}
        <div className="relative flex h-[600px] w-full items-center justify-center lg:h-[500px]">
          <OrbitSystem />
        </div>
      </div>
    </section>
